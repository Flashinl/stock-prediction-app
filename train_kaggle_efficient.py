#!/usr/bin/env python3
"""
Memory-Efficient Kaggle Stock Market Training
Samples the large dataset to fit in memory while maintaining quality
"""
...
class EfficientKaggleTrainer:
    def __init__(self, max_samples=500000):  # Limit to 500K samples
        self.model = None
    ...
    def load_kaggle_stock_data(self):
        """Load and process Kaggle stock data with sampling for memory efficiency"""
        logger.info(f"Loading Kaggle stock data (max {self.max_samples} samples)...")
        ...
        if os.path.exists(stocks_dir):
            ...
            logger.info(f"Processing up to {len(stock_files)} stock files...")
            
            for i, stock_file in enumerate(stock_files):
                if i % 1000 == 0:
                    logger.info(f"Processed {i}/{len(stock_files)} stocks, collected {len(all_stock_data)} samples...")
                
                # Stop if we have enough samples
                if len(all_stock_data) >= self.max_samples:
                    logger.info(f"Reached maximum samples limit ({self.max_samples})")
                    break
                ...
                try:
                    stock_samples = self._process_kaggle_stock_file_with_sampling(
                        os.path.join(stocks_dir, stock_file), symbol
                    )
                    if stock_samples:
                        # Limit samples per stock to prevent memory issues
                        max_per_stock = min(50, len(stock_samples))  # Max 50 samples per stock
                        sampled_stock_data = random.sample(stock_samples, max_per_stock)
                        
                        all_stock_data.extend(sampled_stock_data)
    ...
    def _process_kaggle_stock_file_with_sampling(self, file_path, symbol):
        """Process stock file with sampling for memory efficiency"""
        try:
            ...
            # Create training samples with larger steps to reduce memory usage
            window_size = 60  # 60 days of data to predict next 30 days
            prediction_horizon = 30  # Predict 30 days ahead
            step_size = 10  # Skip 10 days between samples (instead of 1)
            ...
            return samples if len(samples) > 5 else None  # Need at least 5 samples per stock
    ...
    def train_model(self, stock_data_list):
        """Train Random Forest model with sampled data"""
        ...
        try:
            ...
            joblib.dump(self.model, 'models/kaggle_efficient_model.joblib')
            joblib.dump(self.scaler, 'models/kaggle_efficient_scaler.joblib')
            
            # Save feature names
            with open('models/kaggle_efficient_features.json', 'w') as f:
                json.dump(feature_names, f)
            
            logger.info("Model saved successfully")
            
            # Return True if accuracy is good
    ...
    def save_training_report(self, stock_data_list, accuracy=None):
        ...
        try:
            ...
            report_file = f'reports/kaggle_efficient_training_{timestamp}.json'
    ...
    def run_training(self):
        ...
        logger.info("Starting memory-efficient Kaggle training...")
        ...
        try:
            ...
            # Get accuracy for report
            accuracy = None
            if hasattr(self, 'model') and self.model is not None:
                # Quick accuracy check on a subset
                try:
                    X_sample = [data['features'] for data in stock_data_list[:1000]]
                    y_sample = [data['target'] for data in stock_data_list[:1000]]
                    X_sample_scaled = self.scaler.transform(X_sample)
                    y_pred_sample = self.model.predict(X_sample_scaled)
            ...
            if success:
                logger.info("Memory-efficient Kaggle training completed successfully!")
                return True
            else:
                logger.error("Training failed or accuracy below threshold")
                return False
...
def main():
    ...
    trainer = EfficientKaggleTrainer(max_samples=500000)  # 500K samples
